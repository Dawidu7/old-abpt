...
export default function Calculator() {
  ...
  useEffect(() => {setResolution(Math.round(((camera?.pixel_size / telescope?.focal_length * 206.265) * flattReduc ? flattReduc.times : 1) * 100) / 100)}, [camera, telescope, flattReduc])
  useEffect(() => {
    const fov_x = Math.round(((camera?.res_x * resolution / 3600) * flattReduc ? flattReduc.times : 1) * 100) / 100
    const fov_y = Math.round(((camera?.res_y * resolution / 3600) * flattReduc ? flattReduc.times : 1) * 100) / 100
